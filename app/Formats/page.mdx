...
When you provide geographic coordinates as locations to Tomorrow.io endpoints, they should follow the **decimal degrees format of the WGS84 coordinate system**.

A location can be queried in one of the following formats:

---

### 1. `geometry`

The `geometry` attribute of a GeoJSON object according to **RFC 7946**.  
...
For GeoJSON, the coordinate order must be:

---

### 2. `latlong`

A latitude–longitude pair of numbers according to **ISO 6709**.

- As array elements in **body parameters**
- Or separated by a comma in **query parameters**

---

### 3. `locationId`

A string representing the **unique identifier of a location** that was created in the Locations API (from one of the geometry inputs mentioned above).
...
> ❗ **Note if using "geometry" format**
>
> When using these formats _in the docs_ to create the API call, do not use the `"location"` callout. The code snippet the docs generate will add that in for you!
>
> If pasting directly into a snippet of code, you'll need to include the full snippet below with `"location"` included.
>
> Example — to paste into the `"location"` section of any API call in the docs:
>
> ```json
> {
>   "type": "Point",
...
<Tabs items={['geometry', 'latlong (body)', 'latlong (query)', 'locationId']}>
  <Tabs.Tab>
...
````
  ...
  <Tabs.Tab>
    ```json
    {
  "location": "5e82fb82b66492001218aaf3"
}
    ```
  </Tabs.Tab>
</Tabs>
...
Unless otherwise specified, dates and times requested and returned by Tomorrow.io APIs are represented in ISO 8601 format, which can be parsed by many libraries and languages. The timestamps that the API returns are always in the UTC.

Additional datetime formats we support:

- now
- nowPlus`X`m/h/d where `X` is a decimal number. For example, `nowPlus3h` will be the time 3 hours from now.
- nowMinus`X`m/h/d where `X` is a decimal number. For example, `nowMinus15m` will be the time 15 minutes earlier from now.

### Unit System

Version 4.0 currently supports the <span style={{ textDecoration: "underline", color: "#0070f3" }}>Metric </span> and imperial unit systems

### Delimeters

The delimiter `,` separates between array items (like `insights`, `location` latlong `pair`, and `fields`).

### Nulls

There is a shortlist of fields that might be nullified when no other numerical value is applicable:

- `cloudBase` and `cloudCeiling`: there are no clouds
- `windDirection`: there is no wind

### Timezone

Timezone names follow the <span style={{ textDecoration: "underline", color: "#0070f3" }}>IANA Time Zone Database format</span>, such as "America/New_York", as listed below:

<Tabs items={["IANA Timezones"]}>
<Tabs.Tab>
<div style={{ maxHeight: "300px", overflowY: "auto" }}>
      ```json
      {
  	"auto": "automatically set the timezone of the request location"
