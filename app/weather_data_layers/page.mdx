...
| Timestep | Interval Span | Premium | Free |
|----------|---------------|---------|------|
|`1d`|1 day|[-7d,+15d]|[-24h,+5d]|
|`1h`|1 hour|[-7d,+15d]|[-24h,+5d]|
|`30m`|30 minutes|[-7d,+6h]|[-6h,+6h]|
|`15m`|15 minutes|[-7d,+6h]|[-6h,+6h]|
|`5m`|5 minutes|[-7d,+6h]|[-6h,+6h]|
|`1m`|1 minute|[-7d,+6h]|[-6h,+6h]|
|current|realtime|[0,0]|[0,0]|

*Note: For 1m, 5m and 15m timesteps requests are limited to a 24-hour period per API call.
...
- Map ( M ): raster representation of a single field for a given tile
...
- Max (∧): maximum - supported for any location type, non-"1d" timestep for point locations will resolve the raw value. Example: temperatureMax
- Min (∨): minimum - supported for any location type, non-"1d" timestep for point locations will resolve the raw value. Example: temperatureMin
...
> In any cases where a field is not available in a requested timestep, the request will result in a Timestep Bounded Field warning. For example, rainAccumulation is supported only for 1h and above; requesting 1m will result in a warning.

> In any cases where a field is not available in a requested area, the request will result in a Location Bounded Field warning and will not be included in the response. For example, hailBinary is supported only for the US and Canada; requesting hailBinary over Europe will result in a warning.
...
Note that some fields that are described by an object are essentially enums. You will also notice that fields ending with a Time prefix are not listed, as they will always be returned in an ISO 8601 format, according to the required timezone.

<Tabs items={['Metric Values', 'latlong (body)']}>
<Tabs.Tab>
